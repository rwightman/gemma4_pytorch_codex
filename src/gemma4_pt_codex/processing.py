...
class Gemma4Processor:
    """Prepare Gemma4 text+image prompts using the original JAX token expansion scheme."""
    ...
    def __call__(
        ...
        """Tokenize prompt text and preprocess images into a model-ready batch."""
        ...
        for prompt_text, image_group in zip(prompts, image_groups):
            ...
            if image_group is None:
                self._raise_if_prompt_contains_visible_image_token(token_ids)
                expanded_input_ids.append(token_ids)
                image_batches.append(None)
                continue
            ...
            soft_token_counts = image_batch.num_soft_tokens_per_image.reshape(-1).tolist()
            expanded_input_ids.append(
                self.expand_image_placeholders(
                    token_ids,
                    soft_token_counts,
                )
            )
