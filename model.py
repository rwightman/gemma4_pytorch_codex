...
from .layers import build_positions_from_mask, make_causal_bidirectional_mask, merge_flat_embeddings
...
class Gemma4Model(nn.Module):
    ...
    def encode_images_to_text(
        ...
        return self.vision.encode_to_text(patches_or_images, positions_xy, output_length_overrides)
        ...
        *,
        ...
        if kv_cache is None:
            ...
            if self.config.text.use_bidirectional_attention == "vision":
                ...
                full_attention_mask = make_causal_bidirectional_mask(attention_mask, bidirectional_mask=attention_mask)
        ...
        if self.config.text.final_logit_softcap is not None:
            logits = torch.tanh(logits / self.config.text.final_logit_softcap) * self.config.text.final_logit_softcap
        return Gemma4Output(
            logits=logits,
    ...
    def generate(
        ...
        *,
        ...
        next_logits = output.logits[torch.arange(output.logits.shape[0], device=input_ids.device), last_token_index]
        ...
        for _ in range(max_new_tokens):
            ...
            if eos_token_id is not None:
                next_token = torch.where(finished, next_token.new_full((), eos_token_id), next_token)
